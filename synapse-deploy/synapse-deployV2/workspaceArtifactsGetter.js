...
function getArtifactsFromWorkspaceOfType(artifactTypeToQuery, targetWorkspaceName, environment) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        var token = yield (0, deployutils_1.getAuthToken)(params.connectedService, audienceUrl, environment, client);
        var headers = {
        ...
        while (moreResult) {
            ...
            for (let artifactJson of list) {
                ...
                if (type !== artifacts_enum_1.DataFactoryType.database && (0, CommonUtils_1.isDefaultArtifact)(artifactJsonContent)) {
...
function getArtifactsToDeleteFromWorkspaceInOrder(artifactsToDelete) {
    ...
    while (count < artifactsToDelete.length && iteration < MAX_ITERATIONS) {
        ...
        for (var res = 0; res < artifactsToDelete.length; res++) {
            if ((0, armtemplateutils_1.checkIfArtifactExists)(artifactsToDelete[res], artifactsOrdered)) {
                continue;
            }
            ...
            for (let db of databases) {
                ...
                for (let action of ['relationship', 'table']) {
                    ...
                    while (fetchMore) {
                        ...
                        let token = yield (0, deployutils_1.getAuthToken)(params.connectedService, audienceUrl, environment, client);
                        let headers = {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json',
                            'User-Agent': userAgent
