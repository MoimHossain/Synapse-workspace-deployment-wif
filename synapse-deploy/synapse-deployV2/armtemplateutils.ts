...
import {DefaultArtifact, isDefaultArtifact} from "./CommonUtils";
...
export async function getArtifacts(
    ...
    targetLocation: string): Promise<Resource[][]> {
...
function findDefaultArtifacts(armTemplate: string, targetworkspace: string): Map<string, string>
{
...
function replaceParameters(armParams: string, armTemplate: string, overrideArmParameters: string, targetWorkspaceName: string): string {
    ...
    // Replace parameterValues inside parameters first
...
function getParameterValuesFromArmTemplate(
    armParams: string,
    armTemplate: string,
    overrideArmParameters: string,
    ...
    // Convert arm template to json, look at the default parameters if any and add missings ones to the map we have
    let jsonArmTemplateParams = JSON.parse(armTemplate);
...
function getArtifactsFromArmTemplate(armTemplate: string, targetLocation: string, defaultArtifacts: Map<string, string>): Resource[][] {
    ...
    for (var value in jsonArmTemplateParams.resources)
    {
        ...
        if(artifactType.toLowerCase().indexOf(`notebook`) > -1)
        {
            if (!artifactJson.name) {
                /* ideally parameters file is needed incase of Ipynb file.
                 * Update/remove it accordingly
                 */
