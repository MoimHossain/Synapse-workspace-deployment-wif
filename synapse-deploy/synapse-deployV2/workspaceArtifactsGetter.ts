...
import {getAuthToken} from './deployutils';
import * as httpClient from 'typed-rest-client/HttpClient';
import * as httpInterfaces from 'typed-rest-client/Interfaces';
import {checkIfArtifactExists, Database, DbChildren, Resource} from './armtemplateutils';
import {Artifact, DataFactoryType} from './artifacts_enum';
import {isDefaultArtifact} from "./CommonUtils";
...
// Force Node.js to use TLS 1.2 to prevent handshake failures
...
export async function getArtifactsFromWorkspaceOfType(
    artifactTypeToQuery: Artifact,
    targetWorkspaceName: string,
    environment: string): Promise<Resource[]>
{
    var params = await deployUtils.getParams();
    var audienceUrl = ArtifactClient.getAudienceUrl(environment);
    var token = await getAuthToken(params.connectedService, audienceUrl, environment, client);
        ...
        for(let db of databases){
            ...
            for(let action of ['relationship', 'table']){
                ...
                while(fetchMore){
                    ...
                    let audienceUrl = ArtifactClient.getAudienceUrl(environment);
                    let token = await getAuthToken(params.connectedService, audienceUrl, environment, client);
