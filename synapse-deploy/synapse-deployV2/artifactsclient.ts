import { Params, DeployStatus, getAuthToken } from './deployutils';
import * as httpClient from 'typed-rest-client/HttpClient';
import * as httpInterfaces from 'typed-rest-client/Interfaces';
import { v4 as uuidv4 } from 'uuid';
import { Resource } from './armtemplateutils';
...
export class ArtifactClient {
    ...
    constructor(params: Params, client: httpClient.HttpClient) {
        this.params = params;
        this.client = client;
    ...
    public async deployArtifact(resourceType: string, payload: Resource, workspace: string, environment: string): Promise<string> {
        ...
        var token = await getAuthToken(
            this.params.connectedService,
            ArtifactClient.getAudienceUrl(environment),
            environment,
            this.client);
        switch (resourceType) {
    ...
    public async deleteDatalakeChildren(resource: string, workspace: string, location: string): Promise<string>{
        ...
        let token = await getAuthToken(
            this.params.connectedService,
            ArtifactClient.getAudienceUrl(location),
            location,
            this.client);
    ...
    private async deployNotebook(baseUrl: string, payload: Resource, token: string)
        : Promise<string> {
        try {
            return await this.artifactDeploymentTask(baseUrl,
    ...
    private async artifactDeploymentTask(baseUrl: string, resourceType: string, payloadObj: Resource,
        ...
        return new Promise<string>(async (resolve, reject) => {
            var url: string = this.buildArtifactUrl(baseUrl, resourceType, payloadObj.name);
            var payload: string = payloadObj.content;
    ...
    public static getAudienceUrl(env : string): string {
        switch (env) {
